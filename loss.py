...
def CDANE(input_list, ad_net, random_layer, entropy=None, coeff=None, args=None):
    ...
    if random_layer is not None:    
        ...
        '''
        features:  torch.Size([64, 248])
        softmax_output:  torch.Size([64, 2])
        现在:  op_out:  torch.Size([64, 2, 248])
              op_out.view(-1, softmax_output.size(1) * feature.size(1)): torch.Size([64, 496])
              ad_out: torch.Size([64, 1])
        '''
    else:
    ...
    '''
    构建domain label, 维度是(1, bs*2), 如果batchsize是32, 那就是32个1和32个0组成的64维向量
    '''
...
class RandomLayer(nn.Module):
    ...
    def forward(self, input_list):
        # input_list: 2, 64, 248
        # return_list: 2, 64, 248
        # return_tensor: 64, 248
        # print("++++++++++++++++++++================================================================")
        # print('input_list: ', len(input_list[0][0]))
        return_list = [tr.mm(input_list[i], self.random_matrix[i]) for i in range(self.input_num)]
        # print('return_list: ', len(return_list[0][0]))
        return_tensor = return_list[0] / math.pow(float(self.output_dim), 1.0 / len(return_list))
        for single in return_list[1:]:
            return_tensor = tr.mul(return_tensor, single)
        # print('return_tensor: ', len(return_tensor[0]))
        # print("++++++++++++++++++================================================================")
...
class ClassConfusionLoss(nn.Module):
    ...
    def forward(self, output: tr.Tensor) -> tr.Tensor:
        ...
        softmax_out = nn.Softmax(dim=1)(output / self.t)    # torch.Size([32, 2])
        # mapped_output = torch.sigmoid(output / 3) * (torch.tensor([torch.pi / 8])).to(output.device)
        # softmax_out = torch.sin(mapped_output / 1)
        entropy_weight = Entropy(softmax_out).detach()    #  torch.Size([32])
        entropy_weight = 1 + tr.exp(-entropy_weight)     #  torch.Size([32])
        entropy_weight = (n_sample * entropy_weight / tr.sum(entropy_weight)).unsqueeze(dim=1)    #  torch.Size([32, 1])
        class_confusion_matrix = tr.mm((softmax_out * entropy_weight).transpose(1, 0), softmax_out)  # (2,32)*(32,2)=(2,2)
        class_confusion_matrix = class_confusion_matrix / tr.sum(class_confusion_matrix, dim=1)   #  torch.Size([2, 2])
